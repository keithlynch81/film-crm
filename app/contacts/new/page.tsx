...
export default function NewContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user?.id) {
        const contactName = `${data.first_name}${data.last_name ? ' ' + data.last_name : ''}`
    ...
    <Layout>
      <div style={{ maxWidth: '768px' }}>
        <div style={{ marginBottom: '24px' }}>
          <h1 style={{ fontSize: '30px', fontWeight: 'bold', color: '#111827', margin: '0 0 8px 0' }}>Add New Contact</h1>
          <p style={{ fontSize: '14px', color: '#6b7280', margin: 0 }}>
            Add a new industry contact to your workspace.
          </p>
        </div>
