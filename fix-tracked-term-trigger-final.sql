-- Fix tracked term matching trigger with correct notification schema
-- Issues fixed:
-- 1. content_snippet → summary
-- 2. Correct notification schema (title, action_type, entity_type, entity_id, entity_title)

-- Update the trigger function to use 'summary' instead of 'content_snippet'
-- and create notifications with correct schema
...
BEGIN
  ...
  LOOP
    ...
    -- Check if term appears in the new article's title or summary (not content_snippet)
